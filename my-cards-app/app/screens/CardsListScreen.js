...
export default function CardsListScreen() {
  ...
  useEffect(() => {
    fetch(API_URL)
      .then((res) => res.json())
      .then((data) => {
        setCards(data.results || data); // handle paginated or plain list
        setLoading(false);
      })
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => router.push({ pathname: '/CardDetailScreen', params: { cardId: item.id } })} style={styles.card}>
            {item.image ? (
